...
function listAllItems() {
    connection.query("select * from products", async function(err, res) {
        ...
        connection.query('update products set stock_quantity = ? where item_id = ?', [parseInt(finalOrder.originalQuantity - finalOrder.numberOrdered), parseInt(finalOrder.item_id)], function (error, results, fields) {
            if(error) {
            ...
            connection.query('select * from products where item_id = ?', finalOrder.item_id, function(err, res, fields) {
                ...
                connection.query("update products set product_sales = ? where item_id = ?", [newTotalSales, finalOrder.item_id], (err, results, fields) => {
                    if(err) throw err;
                    connection.end();
                });
            });
